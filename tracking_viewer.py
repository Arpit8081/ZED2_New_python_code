...
class GLViewer:
    ...
    def updateData(self, zed_rt, str_t, str_r, state, pc):
        self.mutex.acquire()
        self.pose = zed_rt
        #self.zedPath.add_point_clr(zed_rt.get_translation().get(), [0.1,0.36,0.84])
        self.zedPath.add_point_clr(zed_rt.get_translation().get(), [0.,0.,0.])
        self.trackState = state
        self.txtT = str_t
        self.txtR = str_r
        self.point_cloud.setPoints(pc)
        self.mutex.release()
    def updateData1(self, pc):
        self.mutex.acquire()
        self.point_cloud.setPoints(pc)
        self.mutex.release()
    ...
    def draw(self): 
        ...
        #vpMatrix = self.camera.getViewProjectionMatrixRT(self.pose) #"breaks" viewpoint
